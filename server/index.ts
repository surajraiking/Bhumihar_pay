...
app.post("/v1/webhooks/cashfree", express.raw({ type: "application/json" }), async (req, res, next) => {
  try {
    ...
    const valid = verifyCashfreeWebhook(rawBody, req.header("x-webhook-signature"), req.header("x-webhook-timestamp"));
    if (!valid) return res.status(401).json({ error: "INVALID_WEBHOOK_SIGNATURE" });

    await processCashfreeWebhook(JSON.parse(rawBody));
...
app.listen(env.PORT, () => {
  console.log("Suraj Rai Pay API listening on port " + env.PORT);
});
